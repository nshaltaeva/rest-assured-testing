...
public class ORDSTestsDay4 {
    ...
    public void WarmUp(){
        given().accept("application/json").get("/employees").
                ...
                and().time(lessThan(3L),TimeUnit.SECONDS).log().all(true);
    }

    @Test
    @DisplayName("Verify that response time is less than 3 seconds")
    public void WarUpFromVasyl(){
        given().
        ...
        then().
                ...
                log().all(true);// payload = body and our body has JSON body
...
*/
    ...
    public void test2() {
        given().
                accept(ContentType.JSON).queryParam("q","{\"country_id\":\"US\"}").
        when().get("/countries").
        then().assertThat().
                contentType(ContentType.JSON).
                statusCode(200).
    ...
    public void test4(){
        List<?> countries = given().accept(ContentType.JSON).
    ...
    public void test5(){
        //to use List.of() set java9 at least
